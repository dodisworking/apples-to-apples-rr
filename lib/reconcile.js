...
export function reconcile({ argus, client, learnings = [] }) {
  ...
  // Standard accounting rent rolls prefix every suite with a building/property
  // code (e.g. "OR702-220", "OR702-PRK"). When the Argus roll is a single
  // building, that prefix is pure noise — left in place it makes EVERY suite
  // look like a discrepancy and breaks suite matching. Detect a prefix shared
  // across the client's suites and strip it before matching. Single-building
  // only: if the client roll actually spans multiple buildings, no single
  // prefix dominates, so nothing is stripped (and the user must split rolls).
  const buildingPrefix = detectBuildingPrefix(
    clientActive.map(t => t.suite),
    argusActive.map(t => t.suite),
  )
  if (buildingPrefix) {
    const pfx = buildingPrefix + '-'
    for (const t of clientActive) {
      if (t.suite && String(t.suite).startsWith(pfx)) {
        t.suiteOriginal = t.suite
        t.suite = String(t.suite).slice(pfx.length)
        t.suiteKey = normalizeSuite(t.suite)
      }
    }
  }
  ...
  for (const t of clientOnly) {
    // 0/1-SF client-only rows are misc-revenue suites (parking, antenna,
    // telecom, storage, signage). They are LEGITIMATELY absent from the Argus
    // rent-roll grid because Argus carries that income as misc revenue, NOT as
    // a leased suite. Per reviewer guidance they must STILL be reported (so the
    // user can reconcile them to Argus misc revenue separately) — but they are
    // NOT a missing-tenant error, so flag them LOW/informational and never as a
    // scary HIGH "missing from Argus" discrepancy.
    const isMiscRevenue = (t.sqft || 0) <= 1
    const diff = isMiscRevenue
      ? {
          field: 'tenant_presence', severity: 'LOW',
          argusValue: '— (not in Argus grid)', clientValue: t.name,
          label: 'Misc-revenue suite (client only)',
          rule: '0/1-SF suite (parking/antenna/telecom/storage) — not a leased suite in Argus',
          explain: `This is a 0/1-SF suite (e.g. parking, antenna, telecom, storage or signage). Argus typically carries this income as MISC REVENUE rather than as a leased suite, so its absence from the Argus rent-roll grid is expected — not a missing tenant. Do NOT drop it: report it here so it can be reconciled separately against Argus's misc-revenue line.`,
          miscRevenue: true,
    ...
    matches.push({
      ...
      diffs: [diff],
      flags: { argusOnly: false, clientOnly: true, clean: false, miscRevenue: isMiscRevenue },
    })
  }
...
// Find a systematic building/property prefix on the client's suite labels.
// Returns the prefix token (without the trailing hyphen) or null.
// Safe by design: only fires when ONE prefix is shared by ≥ half of all client
// suites, and never strips a token that is itself a real Argus suite.
function detectBuildingPrefix(clientSuites, argusSuites) {
  const suites = (clientSuites || []).map(s => (s == null ? '' : String(s)))
  const hyphenated = suites.filter(s => s.includes('-'))
  if (hyphenated.length < 2) return null
  const counts = new Map()
  for (const s of hyphenated) {
    const p = s.slice(0, s.indexOf('-')).trim()
    if (p) counts.set(p, (counts.get(p) || 0) + 1)
  }
  let best = null, bestN = 0
  for (const [p, n] of counts) if (n > bestN) { best = p; bestN = n }
  if (!best) return null
  // Must be a systematic prefix (covers ≥ half of all client suites), not a
  // one-off hyphenated suite like "100-A".
  if (bestN / Math.max(1, suites.length) < 0.5) return null
  // Never strip a prefix that is itself a real Argus suite/unit number.
  const argusKeys = new Set((argusSuites || []).map(normalizeSuite).filter(Boolean))
  if (argusKeys.has(normalizeSuite(best))) return null
  return best
}
