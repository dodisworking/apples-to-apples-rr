...
export function reconcile({ argus, client, learnings = [] }) {
  ...
  // Cross-tenant document-coverage signals. An actual in-place rent roll often
  // doesn't itemize concessions at all, while a forecast/valuation model assigns
  // free rent to most speculative leases. If one side carries NO free-rent data
  // anywhere, a per-tenant "1 vs 0" count mismatch is a document-coverage
  // difference (noise), NOT a dropped concession — so we suppress it. When the
  // side DOES carry free rent elsewhere, an absence on one tenant is meaningful
  // and still flags (e.g. the deal team genuinely dropped a concession).
  const ctx = {
    clientHasAnyFreeRent: clientActive.some(t => (t.freeRent || []).length > 0),
    argusHasAnyFreeRent:  argusActive.some(t => (t.freeRent || []).length > 0),
  }
...
function compareTenants(a, c, ctx = {}) {
  ...
  if (aFR.length || cFR.length) {
    if (aFR.length !== cFR.length) {
      ...
      const clientAbsent = cFR.length === 0 && ctx.clientHasAnyFreeRent === false
      const argusAbsent  = aFR.length === 0 && ctx.argusHasAnyFreeRent === false
      ...
      diffs.push({
        ...
        ...(systematic ? {
          ...
          suppressedReason: clientAbsent
            ? 'Client roll itemizes no free rent for any tenant — concession data not tracked in this document (not a dropped concession).'
            : 'Argus carries no free rent for any tenant — concession data not present on the Argus side.',
        } : {}),
      })
    }
