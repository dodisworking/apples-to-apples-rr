...
function compareTenants(a, c) {
  ...
  // and clearly worded — not a HIGH count alarm.
  pairs.unpairedA.forEach((s) => {
    const amt = bestStepPsfAnnual(s, a.sqft)
    diffs.push({
      field: 'rent_step_unmatched', severity: 'LOW',
      label: 'Rent Step only on Argus',
      argusValue: `${s.effectiveDate || '?'}${amt != null ? ` · $${amt.toFixed(2)}/SF/yr` : ''}`,
      clientValue: '— (no step at this date)',
      rule: 'Argus lists a rent step with no matching-date step on the client roll',
      explain: `Argus shows a rent step effective ${s.effectiveDate || '(undated)'} that has no same-date counterpart on the client roll. This is frequently just a presentation difference — the client roll may list only the next step, may have folded this escalation into its current base rent, or may show it under a different date convention. Confirm whether the client lease actually omits this step before treating it as real.`,
    })
  })
  pairs.unpairedC.forEach((s) => {
    const amt = bestStepPsfAnnual(s, c.sqft)
    diffs.push({
      field: 'rent_step_unmatched', severity: 'LOW',
      label: 'Rent Step only on client RR',
      argusValue: '— (no step at this date)',
      clientValue: `${s.effectiveDate || '?'}${amt != null ? ` · $${amt.toFixed(2)}/SF/yr` : ''}`,
      rule: 'Client roll lists a rent step with no matching-date step on Argus',
      explain: `The client roll shows a rent step effective ${s.effectiveDate || '(undated)'} that has no same-date counterpart in Argus. Often a presentation difference — Argus may express this as current base rent, or the client may be listing a renewal-option escalation (a step beyond the lease expiration should be deferred, not flagged). Verify against the lease before treating it as real.`,
    })
  })
...
function pairRentSteps(aSteps, cSteps, aSqft, cSqft) {
  ...
  // If neither side carries parseable dates we can't date-match — fall back to
  // index pairing (still translating amounts to a common axis).
