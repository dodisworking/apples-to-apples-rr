...
export function reconcile({ argus, client }) {
  ...
  for (const pair of matched) {
    const diffs = compareTenants(pair.argus, pair.client)
    matches.push({
      suite: pair.argus.suite || pair.client.suite,
      suiteKey: pair.argus.suiteKey || pair.client.suiteKey,
      argus: pair.argus, client: pair.client,
      matchedBy: pair.matchedBy,
      diffs,
      flags: { argusOnly: false, clientOnly: false, clean: diffs.length === 0 },
    })
  }
  ...
  // 3) Sort by suite
  matches.sort((a, b) => naturalCompare(a.suite, b.suite))
  ...
  const summary = {
    ...
    highSeverityCount: matches.reduce((n, m) => n + m.diffs.filter(d => d.severity === 'HIGH').length, 0),
  }
