...
4. DATES: a client step may anchor to the lease anniversary (e.g. Aug 1) while Argus anchors to calendar/fiscal (e.g. Jan 1). Within ~45 days = the same step → date difference is a FALSE POSITIVE. >45 days apart and clearly a different escalation → may be REAL.
...
export async function verifyFindings(matches, onProgress) {
  ...
  for (let i = 0; i < withFindings.length; i++) {
    ...
    for (const d of (m.diffs || [])) {
      ...
      // A tenant present on only one roll is a hard fact, not a translation
      // nuance — never auto-suppress it. Keep it as a live finding (with the
      // verifier's opinion attached) so the reviewer always confirms/rejects it.
      if (d.field === 'tenant_presence') { keep.push(d); continue }
