...
- SUITE / UNIT: the reconciler already normalizes suites before comparing (it strips leading zeros, "Suite "/"Ste " prefixes, and hyphen/space formatting — so "0820"="820" and "1A"="Suite 1-A" never flag). Therefore if it flags a "suite" finding, the identifiers genuinely differ AFTER normalization (e.g. "1" vs "2") — that is a REAL discrepancy. CONFIRM it. Do NOT suppress a suite finding just because the names or SF match; a tenant in a different unit number is exactly what the reviewer needs to see.
...
export async function verifyFindings(matches, onProgress) {
  ...
  for (let i = 0; i < withFindings.length; i++) {
    ...
    for (const d of (m.diffs || [])) {
      ...
      if (d.field === 'tenant_presence' || d.field === 'suite') { keep.push(d); continue }
