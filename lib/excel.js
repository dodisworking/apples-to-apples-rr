...
function buildReviewReportSheet(wb, data) {
  ...
  for (const m of data.matches || []) {
    ...
    for (const f of findings) allFindings.push({ m, reviewFor, f })
  }

  // Order so the sheet LEADS with what truly differs: real findings first,
  // AI-suppressed noise sinks to the bottom; within each group, HIGH → MED → LOW.
  const sevRank = { HIGH: 0, MEDIUM: 1, LOW: 2 }
  const isNoise = e => !!(e.f.suppressed && e.reviewFor(e.f.field).verdict !== 'good')
  allFindings.sort((a, b) => {
    const an = isNoise(a) ? 1 : 0, bn = isNoise(b) ? 1 : 0
    if (an !== bn) return an - bn
    return (sevRank[a.f.severity] ?? 3) - (sevRank[b.f.severity] ?? 3)
  })
  const realCount = allFindings.filter(e => !isNoise(e)).length

  let dividerDrawn = false
  for (const entry of allFindings) {
    const { m, reviewFor, f } = entry
    // Banner between the real differences and the AI-suppressed noise section.
    if (!dividerDrawn && realCount > 0 && allFindings.length > realCount && isNoise(entry)) {
      dividerDrawn = true
      ws.mergeCells(r, 1, r, cols.length)
      cell(ws, r, 1, '▼ Below: findings the AI auto-suppressed as likely noise (not real differences) — shown for audit only',
        { fill: GRAY_BG, color: 'FF64748B', bold: true, align: 'center', size: 10 })
      ws.getRow(r).height = 20
      r++
    }
    findingNum++
    const rev = reviewFor(f.field)
    const suppressed = f.suppressed && rev.verdict !== 'good'
    ...
    const statusFill = status === 'CONFIRMED'  ? LIGHT_GRN
                     ...
                     : status === 'SUPPRESSED' ? GRAY_BG
                     : LIGHT_ORG
    const statusClr  = status === 'CONFIRMED'  ? MATCH_GRN
                     : status === 'REJECTED'   ? MISS_RED
                     : status === 'SUPPRESSED' ? 'FF94A3B8'
                     : 'FFC2410C'
    const sevFill = suppressed              ? GRAY_BG
                  : f.severity === 'HIGH'   ? LIGHT_RED
                  : f.severity === 'MEDIUM' ? LIGHT_ORG
                  : 'FFFEF9C3'

    const tenantA = m.argus?.name  || ''
    const tenantC = m.client?.name || ''
    const explainText = suppressed
      ? `(Auto-suppressed as likely noise — not a real difference.${f.suppressedReason ? ' ' + f.suppressedReason : ''})\n${f.explain || ''}`
      : (f.explain || '')
    ...
    const vals = [
      ...
      { v: explainText,                                      wrap: true, size: 9 },
    ...
    const longest = Math.max(
      ...
      Math.ceil((explainText || '').length / 80),
