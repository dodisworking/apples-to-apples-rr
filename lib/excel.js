  ...
  for (const m of data.matches || []) {
    ...
    for (const f of findings) {
      ...
      const vals = [
        ...
        { v: rev.verdict === 'good' ? '👍 CONFIRMED' : rev.verdict === 'bad' ? '👎 REJECTED' : '',
          fill: rev.verdict === 'good' ? LIGHT_GRN : rev.verdict === 'bad' ? LIGHT_RED : undefined,
          color: rev.verdict === 'good' ? MATCH_GRN : rev.verdict === 'bad' ? MISS_RED : undefined,
          bold: true, align: 'center' },
        { v: rev.note || '',                                   wrap: true },
      ...
      // Row height grows with the longest wrapped text (rule / explain)
