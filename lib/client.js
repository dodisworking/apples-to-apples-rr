...
export async function normalizeClientEnsemble({ parsed, filename, model = 'claude-sonnet-4-6', onStage } = {}) {
  ...
  for (const a of passA.tenants) {
    ...
    if (b) {
      usedB.add(k)
      if (tenantsAgree(a, b)) {
        // Both AI passes independently produced the same values for this tenant —
        // high confidence the standardization is right here.
        a._standardization = { status: 'agreed' }
        agreed.push(a)
      } else {
        disputes.push({ key: k, suite: a.suite || b.suite, a, b })
      }
    } else {
  ...
  // Capture WHERE the two passes disagreed per tenant, so each downstream finding
  // can carry the provenance ("the two standardizations disagreed on this field").
  // This is the gold signal for tuning the model: a disagreement means the
  // standardization step is unstable on that tenant/field for this RR format.
  const disputeByKey = new Map(
    disputes.map(d => [d.key, {
      onlyFoundBy: d.a && d.b ? null : (d.a ? 'passA' : 'passB'),
      fields: (d.a && d.b) ? disputeFields(d.a, d.b) : null,
    }])
  )
  ...
  for (const t of resolved) {
    const k = keyOf(t)
    const dispute = disputeByKey.get(k)
    // Tag tie-broken tenants with what the two passes disagreed on — exported
    // per-finding so the feedback doc shows where standardization was unstable.
    t._standardization = { status: 'tiebroken', ...(dispute || {}) }
    finalByKey.set(k, t)
  }
...
// Field-by-field disagreement between two extractions of the same tenant —
// reported on tie-broken tenants so the export shows exactly which standardized
// fields the model was unstable on (the actionable signal for tuning).
function disputeFields(a, b) {
  const out = []
  if (!nameClose(a.name, b.name) && simplifyName(a.name) !== simplifyName(b.name))
    out.push({ field: 'tenant_name', passA: a.name ?? null, passB: b.name ?? null })
  if (!numClose(a.sqft, b.sqft, 1))
    out.push({ field: 'sqft', passA: a.sqft ?? null, passB: b.sqft ?? null })
  if (dnorm(a.leaseStart) !== dnorm(b.leaseStart))
    out.push({ field: 'lease_start', passA: a.leaseStart ?? null, passB: b.leaseStart ?? null })
  if (dnorm(a.leaseEnd) !== dnorm(b.leaseEnd))
    out.push({ field: 'lease_end', passA: a.leaseEnd ?? null, passB: b.leaseEnd ?? null })
  if (!numClose(psfOf(a), psfOf(b), 0.02))
    out.push({ field: 'base_rent_psf', passA: psfOf(a), passB: psfOf(b) })
  if (!stepsAgree(a.rentSteps, b.rentSteps, a.sqft, b.sqft))
    out.push({ field: 'rent_steps', passA: a.rentSteps ?? null, passB: b.rentSteps ?? null })
  return out
}
